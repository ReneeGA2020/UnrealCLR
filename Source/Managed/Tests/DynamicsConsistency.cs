...
public class DynamicsConsistency : ISystem
{
    ...
    private readonly ConsoleVariable variable;
    ...
    private void VariableEvent()
    {
        Debug.AddOnScreenMessage(-1, 5.0f, Color.LightBlue, "Variable has changed: " + variable.GetInt());
    }

    private void ConsoleCommand(float value)
    {
        Debug.AddOnScreenMessage(-1, 5.0f, Color.LightGreen, "Test console command executed: " + value);
    }

    private void PauseResume()
    {
        _ = playerController.SetPause(!playerController.IsPaused);
    }

    private void PlayerCommand()
    {
        playerController.ConsoleCommand(consoleCommand + " " + ++commandsCount);
    }

    private void MouseXMessage(float axisValue)
    {
        Debug.AddOnScreenMessage(1, 3.0f, Color.PaleGoldenrod, "Mouse X axis value: " + axisValue);
    }

    private void MouseYMessage(float axisValue)
    {
